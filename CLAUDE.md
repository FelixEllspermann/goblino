...
- Build Hut (+pop cap), Barracks (trains Club + Archer + Speargoblin), **Docks** (coastal, trains Boats), **Tower** (Defense — auto-fires arrows at hostile units in range once built), or **Wall** (Defense — cheap 1×1 segment that blocks unit movement + auto-tiles its sprite corner/horizontal/vertical from its wall neighbours) — Farmer selects, right-click building card, click to place, Farmer constructs it. Costs are asset-driven (`BuildingDefinition`).
