...
- Build Hut (+pop cap), Barracks (trains Club + Archer + Speargoblin), **Docks** (coastal, trains Boats), **Tower** (Defense — auto-fires arrows at hostile units in range once built), **Wall** (Defense — cheap 1×1 segment that blocks unit movement + auto-tiles its sprite corner/horizontal/vertical from its wall neighbours), or **Horde Hall** (Military, 2×2 — trains the **Minotaur**: very expensive/tanky melee with a big siege bonus vs buildings, even bigger vs walls + towers) — Farmer selects, right-click building card, click to place, Farmer constructs it. Costs are asset-driven (`BuildingDefinition`).
...
- **Asset-driven definitions**: `BuildingDefinition` (ScriptableObject) carries sprite/footprint/cost/trains-units/pop-provided + `WaterSprite` (dock pier) + `Category` (`BuildCategory` enum → build-menu tab) + `Requires` (prereq buildings; empty = always buildable). `GoblinUnitDefinition` carries icon/wood-cost/food-cost/pop-cost/spawn-duration/HP/damage/attack-interval/range + `ProjectileSprite` (set → ranged, fires an `Arrow`), `WorldScale` (monsters scale up), `WaterUnit` (boat: water-only movement), `MoveSpeed` (cells/sec, default 2.0), `BonusVsMonstersAndMelee` (damage multiplier vs monsters + enemy melee, 1 = none — Speargoblin), `KnockbackStrength` (cosmetic on-hit shove, 0 = none), `Armor` (points → diminishing-returns damage reduction `armor/(armor+36)`, capped 80%; Archer 0, Club 4 ≈ 10%, Spear 15 ≈ 29%), `BonusVsArmored` (damage multiplier vs targets with Armor > 0 — Archer = 1.5, armor-piercing; counters Club/Spear), `BonusVsBuildings` / `BonusVsDefensiveBuildings` (siege multipliers vs enemy buildings / walls+towers — the Minotaur; building damage routes through `CombatBonus.EffectiveVsBuilding` in `Goblin.AttackingBuilding`, defenses take the larger multiplier). Per-target attack bonuses live in `CombatBonus` (`Effective` = spear bonus vs monsters/melee, `WithArmorPierce` = archer bonus vs armored — both applied attacker-side in `Goblin.EffectiveDamageAgainst`, used by melee AND the ranged Arrow path); defender-side armor reduction in `ArmorMath` (both pure, in `RTSCL.World`, unit-tested). `ArmorMath.Apply` FLOORS the reduced damage (defender-favoured, so even small armor reliably chips integer hits) with a min-1 floor. Damage flow: attacker applies `CombatBonus` → wire/`EvDamage` → defender's `TakeDamage` applies `ArmorMath` (so armor stays in MP lockstep; reduction never reaches 100%). `BuildingCatalog.asset` lists reachable buildings (incl. Docks); unit assets live under `Assets/Generated/Units/`.
- **Upgrades** (per-owner, paid in iron/gold/crystal): asset-driven via `UpgradeDefinition` (cost + `UpgradeKind` + `MaxLevel`) referenced from `BuildingDefinition.ProvidesUpgrades`; build-menu shows them automatically and **hides a card once fully researched** (leveled ones show "(Lv n/max)" + the next level's cost = base × level). `PlayerUpgrades` tracks a LEVEL per (owner,kind); `UpgradeCatalog` maps kind→def (MaxLevel/costs). Effects in `UpgradeEffects` (per-unit, applied live + retroactively to new spawns via `ApplyExistingTo`, once per level) — except owner-level effects `UnitTrainSpeed` (`TrainingSpeed`), `SightRange` (`SightRange`), `TowerDamage` (`TowerPower`, read live by `TowerCombat`), `WallHp` (`WallStrength` at placement + retroactive `BuildingHP.ScaleMax`). `UpgradeEffects.OnPurchased` is the single entry (player via `NetCommandIssuer`/`NetCommandApplier`, bot via `BotController.TryResearchFrom`, which researches from Keep + Workshop + Mill). **14 stat upgrades:** Keep = Sight Range; Mill = Bountiful Harvest, Farmer Harvest Speed, Farmer Move Speed, Farmer Carry Capacity, Build Speed; Workshop = Club Damage, Club HP, Melee Armor, **Ranged Range (3 levels)**, All-Units Damage, Train Speed, Wall HP, Tower Damage.
- **Research / tech tree**: building unlocks are modeled as upgrades too. The **Research Center** (reuses the Workshops_5 sprite, always buildable) provides "Research X" cards (`UnlockMill/UnlockWorkshop/UnlockWheatfield/UnlockHordeHall` — the Horde Hall costs 200 gold + 200 crystal). A `BuildingDefinition` with `NeedsResearch` + `ResearchKind` is **hidden from the farmer build menu** until that unlock is purchased; locked buildings (any unmet `Requires` or unresearched) are hidden entirely (not greyed). Research chains via `UpgradeDefinition.RequiresUpgrades` (Research Wheat Field is offered only after Research Mill). The bot bypasses the menu gate (builds Mill/Workshop directly). **Researching/upgrading is timed** (`UpgradeDefinition.ResearchDuration`, default 8s): the build menu charges + starts `ResearchProgress` (one per building), the actions-panel progress bar shows it, and `GoblinProductionRunner` applies it on completion. A building offering both units + upgrades (the Keep) splits its actions into **Units / Upgrades sub-tabs**; the actions progress bar always reserves its row height so cards don't shift when production starts/stops.
- **Click-selection** uses each unit's padded sprite **`SelectionBounds`** (not a fixed radius), so large/offset sprites (boats, monsters) are pickable across their hull. Enemy/neutral units are click-inspectable (read-only) but never enter the commandable selection. **Control groups** (`GoblinSelectionController`): Ctrl+digit (1–9, 0) saves the selection; the digit recalls it; a quick double-tap of the same digit also centres the camera (`RTSCamera2D.JumpTo`).
...
| `Assets/Scripts/World/Unity/Arrow.cs` | RTSCL.World.Unity | Homing projectile (unit damage / visual-only building hit / tower-source damage) |
| `Assets/Scripts/World/Unity/WallSegment.cs` / `WallRegistry.cs` | RTSCL.World.Unity | Walls: `WallRegistry` is the global wall-cell set (`Goblin.IsCellPassable` blocks land units on wall cells — the ONLY building type that blocks pathing); `WallSegment` auto-tiles its sprite from wall neighbours (corner/horizontal/vertical) and refreshes neighbours on place/remove. Buildings become walls by setting `BuildingDefinition.WallCornerSprite/WallHorizontalSprite/WallVerticalSprite`. Removal unregisters in `BuildingPlacer.RemoveBuilding` (not just OnDestroy) so destroyed walls stop blocking immediately |
| `Assets/Scripts/World/Unity/TowerCombat.cs` | RTSCL.World.Unity | Defensive-building auto-attack: owner-local, finds nearest hostile in range, fires arrows; damage via `NetCommandIssuer.IssueTowerDamage` (sentinel attacker NetId, reuses `EvDamage`). Buildings get combat from `BuildingDefinition.AttackDamage/AttackRange/AttackInterval/ProjectileSprite` |
