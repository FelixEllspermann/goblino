...
- Pure-logic tests under `Assets/Tests/Editor/` cover the world generator (+ `GoblinNetId`, `BuildRequirements`, `CombatBonus`). 54 tests, all passing. The `RTSCL.World.Tests` asmdef references `RTSCL.World` **and** `RTSCL.World.Unity`. Run via the Test Runner window or via Unity MCP's `TestRunnerApi`.
