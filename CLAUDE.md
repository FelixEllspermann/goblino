...
- Farmers harvest (auto-find same resource kind nearby, fan out via global standing-cell reservations) + build; Clubs melee, **Archers** fire homing arrows, **Speargoblins** are slow tanky melee with **reach** (range 2) that deal **bonus damage vs monsters + enemy melee** (`CombatBonus`, multiplier on `GoblinUnitDefinition`) and a light on-hit knockback — strong vs Clubs, kited by Archers; units **attack & destroy buildings** (melee lunge / ranged arrow + building hit FX). Melee reach units won't strike across un-walkable water/cliff gaps (anti-camping path check).
...
- Pure-logic tests under `Assets/Tests/Editor/` cover the world generator (+ `GoblinNetId`, `BuildRequirements`, `CombatBonus`, `ArmorMath`). 64 tests, all passing. The `RTSCL.World.Tests` asmdef references `RTSCL.World` **and** `RTSCL.World.Unity`. Run via the Test Runner window or via Unity MCP's `TestRunnerApi`.
